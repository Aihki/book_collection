...
const fetchLikesByMediaId = async (id: string): Promise<Like[] | null> => {
  try {
    const [rows] = await promisePool.execute<RowDataPacket[] & Like[]>(
...
const fetchLikesCountByMediaId = async (id: string): Promise<string | null> => {
  try {
    const [rows] = await promisePool.execute<RowDataPacket[] & Like[]>(
      'SELECT COUNT(*) as likesCount FROM Likes WHERE book_id = ?',
      [id],
    );
    if (rows.length === 0) {
      return null;
...
const fetchLikesByUserId = async (id: string): Promise<Like[] | null> => {
  try {
    const [rows] = await promisePool.execute<RowDataPacket[] & Like[]>(
...
const deleteLike = async (
  ...
  try {
    ...
    console.log('deleteLike', likeResult.affectedRows)
